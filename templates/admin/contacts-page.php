...
<div class="wrap cin-contacts-page">
    ...
    <div class="cin-page-header">
        <div>
            <h1><?php esc_html_e('Contacts',  'contactin'); ?></h1>
        ...
        <form method="get" id="contacts-filter" class="cin-contacts-form">
            ...
            <?php if ( ! empty( $search ) ) : ?>
                <div class="cin-active-filters">
                    <span class="cin-filters-label"><?php esc_html_e('Filters:',  'contactin'); ?></span>
                    <span class="cin-filter-badge">
                        <span>🔍</span>
                        <span><?php echo esc_html($search); ?></span>
                        <a href="<?php echo esc_url(remove_query_arg(['s', 'paged'], $base_url)); ?>" title="<?php esc_attr_e('Remove search',  'contactin'); ?>">&times;</a>
                    </span>
                </div>
            <?php endif; ?>
            ...
            <div class="cin-search-export-row">
                <div class="cin-search-controls">
                    <label class="screen-reader-text" for="contacts-search-input"><?php esc_html_e('Search Contacts',  'contactin'); ?></label>
                    ...
                    <?php if ( ! empty( $search ) ) : ?>
                        <a href="<?php echo esc_url(remove_query_arg(['s', 'paged'], $base_url)); ?>" class="button"><?php esc_html_e('Clear',  'contactin'); ?></a>
                    <?php endif; ?>
                </div>
            ...
            <div class="tablenav top">
                <div class="tablenav-pages">
                    ...
                    <?php if ($pages > 1) : ?>
                        <?php $pagination_args = ['base' => add_query_arg('paged', '%#%', $base_url), 'format' => '', 'current' => max(1, $paged), 'total' => max(1, $pages), 'type' => 'plain', 'prev_text' => __('Prev',  'contactin'), 'next_text' => __('Next',  'contactin'), 'add_args' => ['s' => $search, 'per_page' => $per_page, 'orderby' => $orderby, 'order' => $order]]; ?>
                        ...
                        <tr>
                            <th class="sortable <?php echo $orderby === 'name' ? 'sorted' : ''; ?> <?php echo $orderby === 'name' ? strtolower($order) : 'desc'; ?>">
                                <a href="<?php echo esc_url(add_query_arg(['orderby' => 'name', 'order' => ($orderby === 'name' && $order === 'ASC' ? 'DESC' : 'ASC'), 's' => $search, 'per_page' => $per_page, 'paged' => 1], $base_url)); ?>">
                                    <span><?php esc_html_e('Name',  'contactin'); ?></span>
                            ...
                            <th class="sortable <?php echo $orderby === 'last_message_at' ? 'sorted' : ''; ?> <?php echo $orderby === 'last_message_at' ? strtolower($order) : 'desc'; ?>">
                                <a href="<?php echo esc_url(add_query_arg(['orderby' => 'last_message_at', 'order' => ($orderby === 'last_message_at' && $order === 'ASC' ? 'DESC' : 'ASC'), 's' => $search, 'per_page' => $per_page, 'paged' => 1], $base_url)); ?>">
                                    <span><?php esc_html_e('Last Activity',  'contactin'); ?></span>
                                    <span class="sorting-indicator"></span>
                                </a>
                            </th>
                            <th class="table-col-actions"><?php esc_html_e('Actions',  'contactin'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <?php else : ?>
                            <?php foreach ($contacts_list as $contact) : ?>
                                ...
                                <tr id="contactin-row-<?php echo esc_attr($contact->id); ?>">
                                    ...
                                    <td class="column-actions actions" data-label="<?php esc_attr_e('Actions',  'contactin'); ?>">
    ...
    <div id="cin-export-modal" class="cin-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="cin-export-modal-title">
        <div class="cin-confirm-modal">
            <h3 id="cin-export-modal-title"><?php esc_html_e('Export Contacts',  'contactin'); ?></h3>
