...
final class QueueHealthMonitor {
	...
	protected function __construct() {
		...
		add_action( 'contactin_queue_health_check', array( $this, 'run_health_check' ) );
	...
	private function check_and_release_stuck_locks(): int {
		$locks    = array( 'email', 'crm', 'webhook', 'analytics' );
		$released = 0;
		...
		foreach ( $locks as $process ) {
			...
			if ( $age_seconds > 600 ) {
				Logger::warning(
					'Stuck ProcessLock detected - force releasing',
					array(
						'process'     => $process,
	...
	private function check_queue_backup(): array {
		...
		$alerts          = 0;
		$queue_breakdown = array();

		foreach ( $stats as $stat ) {
			if ( ! isset( $queue_breakdown[ $stat->type ] ) ) {
				$queue_breakdown[ $stat->type ] = array(
					'pending'    => 0,
					'processing' => 0,
					'retry'      => 0,
				);
		...
		foreach ( $queue_breakdown as $type => $counts ) {
			...
			if ( $processing > $pending && $pending > 0 ) {
				Logger::warning(
					'Stuck processing items detected',
					array(
						'type'       => $type,
						'processing' => $processing,
						'pending'    => $pending,
	...
	private function ensure_cron_schedules(): int {
		...
		foreach ( $critical_crons as $cron ) {
			...
			if ( ! $next ) {
				...
				Logger::warning(
					'Missing cron schedule detected - rescheduling',
					array(
						'cron' => $cron,
					)
				);
				...
				if ( ! isset( $schedules[ $recurrence ] ) ) {
					...
					Logger::warning(
						'Invalid recurrence while rescheduling cron; using fallback',
						array(
							'cron'               => $cron,
	...
	private function check_stalled_processing(): int {
		...
		foreach ( $stalled as $item ) {
			Logger::warning(
				'Stalled processing item detected - recovering',
				array(
					'queue_id'    => $item->id,
					'type'        => $item->type,
			...
			if ( $wpdb->update(
				...
				array(
					'status'     => 'pending',
					'last_error' => 'Auto-recovered from stalled state',
				),
				array( 'id' => $item->id ),
				array( '%s', '%s' ),
				array( '%d' )
		...
		if ( $recovered > 0 ) {
			Logger::info( 'Recovered stalled items', array( 'count' => $recovered ) );

			AlertGenerator::alert_queue_issue(
				"Auto-recovered {$recovered} stalled queue items",
				'INFO',
				array( 'recovered_count' => $recovered )
			);
		}
