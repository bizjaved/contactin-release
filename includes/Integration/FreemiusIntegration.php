...
final class FreemiusIntegration {
    ...
    * @return object|null The Freemius instance
    ...
    private static function register_hooks(): void {
        ...
        add_action(
            ...
            [ self::class, 'on_premium_activation' ]
        );
        ...
        add_action( 'admin_init', [ self::class, 'maybe_heal_premium_workloads' ] );
    ...
    * @return object|null
     ...
     * @return void
     */
    public static function track_event( string $event_name, array $data = [] ): void {
        if ( null === self::$freemius ) {
            return;
        }
    ...
    public static function is_feature_enabled( string $feature ): bool {
        $normalized_feature = sanitize_key( str_replace( [ '.', ':' ], '_', strtolower( trim( $feature ) ) ) );
    ...
    public static function get_activate_license_url(): string {
        ...
        try {
            ...
            if ( method_exists( self::$freemius, 'get_account_url' ) ) {
                $url = self::$freemius->get_account_url( false, [ 'activate_license' => 'true' ] );
                if ( ! empty( $url ) ) {
                    return self::enforce_premium_renewal_context( $url );
                }
            }
