...
class CRMDataHandler extends BaseAJAXHandler {
	...
	public function handle(): void {
		...
		try {
			...
			$daily_formatted = array();
			if ( is_array( $daily_stats ) ) {
				foreach ( $daily_stats as $item ) {
					$daily_formatted[] = array(
						'date'   => isset( $item['date'] ) ? gmdate( 'M d', strtotime( $item['date'] ) ) : $item['date'] ?? '',
	...
	private function verify_crm_nonce(): void {
		...
		if ( ! $valid ) {
			wp_send_json_error( array( 'message' => __( 'Invalid nonce.', 'contactin' ) ) );
			exit;
		}
