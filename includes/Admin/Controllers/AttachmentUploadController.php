	...
	public static function upload( WP_REST_Request $request ) {
		...
		if ( empty( $settings['form_enable_attachment'] ) ) {
			return new WP_Error(
				...
				__( 'File attachments are disabled in plugin settings.', Config::TEXTDOMAIN ),
				[ 'status' => 403 ]
			);
		}
		...
		// Security: reCAPTCHA verification if enabled
		...
		if ( ! empty( $recaptcha_token ) ) {
			if ( ! Security::verify_recaptcha( $recaptcha_token ) ) {
				...
				return new WP_Error(
					'recaptcha_failed',
					__( 'reCAPTCHA verification failed. Please try again.', Config::TEXTDOMAIN ),
					[ 'status' => 403 ]
				);
			}
		}
		...
		if ( empty( $files['file'] ) ) {
			...
			return new WP_Error(
				'no_file',
				__( 'No file provided.', Config::TEXTDOMAIN ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! is_dir( $temp_dir ) && ! wp_mkdir_p( $temp_dir ) ) {
			...
			return new WP_Error(
				...
				__( 'Failed to create upload directory.', Config::TEXTDOMAIN ),
				[ 'status' => 500 ]
			);
		}
		...
		if ( ! move_uploaded_file( $file['tmp_name'], $file_path ) ) {
			...
			return new WP_Error(
				'file_move_failed',
				__( 'Failed to move uploaded file.', Config::TEXTDOMAIN ),
				[ 'status' => 500 ]
			);
		}
	...
	private static function validate_file( $file, array $settings ) {
		...
		if ( $file['size'] > $max_size ) {
			return new WP_Error(
				'file_too_large',
				sprintf(
					__( 'File size exceeds maximum allowed size of %d MB.', Config::TEXTDOMAIN ),
					$max_size_mb
				),
				[ 'status' => 400 ]
			);
		...
		$allowed_types_raw = $settings['allowed_file_types'] ?? 'jpg,png,gif,pdf,doc,docx';
		...
		if ( ! in_array( $file_ext, $allowed_exts, true ) ) {
			return new WP_Error(
				'file_type_not_allowed',
				   sprintf(
					   __( 'File type .%s is not allowed. Allowed types: %s', Config::TEXTDOMAIN ),
					   $file_ext,
	...
	private static function check_upload_rate_limit() {
		...
		if ( $count >= self::UPLOAD_RATE_LIMIT_MAX ) {
			...
			return new WP_Error(
				...
				sprintf(
					__( 'Too many uploads. Please wait %d minutes before uploading again.', Config::TEXTDOMAIN ),
