...
final class PluginInfo {
    ...
    private string $plugin_slug = 'contactin-pro';
    ...
    protected function init(): void {
        add_filter('wp_kses_allowed_html', [$this, 'extend_allowed_html_tags'], 10, 2);
        add_action('admin_head', [$this, 'print_modal_comparison_styles']);
    }
    ...
    public function extend_allowed_html_tags(array $tags, string $context): array {
        ...
        $tags['table'] = ['class' => true];
        $tags['thead'] = ['class' => true];
        $tags['tbody'] = ['class' => true];
        $tags['tr'] = ['class' => true];
        $tags['th'] = [
            'class' => true,
            'colspan' => true,
    ...
    private function is_fallback_upgrade_modal(object $result): bool {
        ...
        $plugin_name = isset($result->name) ? (string) $result->name : 'ContactIn Pro';
    ...
    private function get_plugin_data(): object {
        ...
        $data = new \stdClass();
        $data->name = 'ContactIn Pro';
        $data->slug = $this->plugin_slug;
        $data->plugin = 'contactin-pro/contactin.php';
        $data->version = $plugin_version;
        $data->author = 'Javed Ahsan';
        $data->author_profile = 'https://linkedin.com/in/bizjaved';
        $data->homepage = 'https://github.com/bizjaved/contactin-pro';
        $data->download_link = '';
        $data->donate_link = '';
        $data->requires = '6.4';
        $data->tested = '6.9.1';
    ...
    private function is_readme_table_row_line(string $line): bool {
        ...
        $non_empty_cells = array_filter($cells, static function (string $cell): bool {
            return trim($cell) !== '';
        });
    ...
    private function render_readme_table(array $rows): string {
        ...
        if (!empty($header)) {
            $header_labels = array_map(function ($cell): string {
    ...
    private function resolve_asset_url_by_extensions(string $relative_path): string {
        ...
        foreach (['jpg', 'jpeg', 'png', 'svg'] as $candidate_ext) {
