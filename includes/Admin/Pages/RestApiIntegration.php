...
final class RestApiIntegration {
	...
	protected function __construct() {
		add_action( 'wp_ajax_contactinbox_generate_rest_token', array( $this, 'ajax_generate_token' ) );
		add_action( 'wp_ajax_contactinbox_revoke_rest_token', array( $this, 'ajax_revoke_token' ) );
		add_action( 'wp_ajax_contactinbox_get_rest_health', array( $this, 'ajax_get_health' ) );
		add_action( 'wp_ajax_contactinbox_test_api_connection', array( $this, 'ajax_test_connection' ) );
		add_action( 'wp_ajax_contactinbox_toggle_restapi_service', array( $this, 'ajax_toggle_restapi_service' ) );
		add_action( 'wp_ajax_contactinbox_save_rate_limits', array( $this, 'ajax_save_rate_limits' ) );

		// Legacy action hooks (contactin_ prefix)
		add_action( 'wp_ajax_contactin_generate_rest_token', array( $this, 'ajax_generate_token' ) );
		add_action( 'wp_ajax_contactin_revoke_rest_token', array( $this, 'ajax_revoke_token' ) );
		add_action( 'wp_ajax_contactin_get_rest_health', array( $this, 'ajax_get_health' ) );
		add_action( 'wp_ajax_contactin_test_api_connection', array( $this, 'ajax_test_connection' ) );
		add_action( 'wp_ajax_contactin_toggle_restapi_service', array( $this, 'ajax_toggle_restapi_service' ) );
		add_action( 'wp_ajax_contactin_save_rate_limits', array( $this, 'ajax_save_rate_limits' ) );
	}
	...
	public function ajax_save_rate_limits(): void {
		...
		if ( ! in_array( $unit, array( 'minute', 'hour', 'day' ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid rate limit unit.', 'contactin' ) ) );
		}
	...
	public static function render(): void {
		...
		if ( isset( $_POST['contactinbox_toggle_restapi'], $_POST['contactinbox_restapi_toggle'] ) ) {
			...
			wp_safe_redirect( add_query_arg( array( 'page' => $_GET['page'] ?? 'contactin-restapi' ), admin_url( 'admin.php' ) ) );
			exit;
		}
	...
	public static function get_active_tokens(): array {
		$tokens = get_option( 'contactin_test_tokens', array() );
	...
	public function ajax_generate_token(): void {
		...
		$scopes = isset( $_POST['routes'] ) ? array_map( 'sanitize_text_field', (array) $_POST['routes'] ) : array();
		$ttl    = isset( $_POST['ttl'] ) ? absint( $_POST['ttl'] ) : 3600; // default 1 hour

		// Default scopes if none specified
		if ( empty( $scopes ) ) {
			$scopes = array( '/contactin/v1/submit' );
		}
	...
	public function ajax_revoke_token(): void {
		...
		if ( empty( $token_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid token ID.', 'contactin' ) ) );
		}
		...
		try {
			...
			if ( $revoked ) {
				wp_send_json_success( array( 'message' => __( 'Token revoked successfully.', 'contactin' ) ) );
			} else {
				wp_send_json_error( array( 'message' => __( 'Token not found or already revoked.', 'contactin' ) ) );
			}
		} catch ( Exception $e ) {
			wp_send_json_error( array( 'message' => $e->getMessage() ) );
		}
	...
	public function ajax_test_connection(): void {
		...
		if ( empty( $name ) || empty( $email ) || empty( $message ) ) {
			wp_send_json_error( array( 'message' => __( 'All fields are required.', 'contactin' ) ) );
		}

		if ( ! is_email( $email ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid email address.', 'contactin' ) ) );
		}
		...
		if ( $has_attachment ) {
			...
			$upload_request->set_file_params( array( 'file' => $_FILES['attachment'] ) );
			...
			$upload_data = ( $upload_response instanceof \WP_REST_Response ) ? $upload_response->get_data() : array();
