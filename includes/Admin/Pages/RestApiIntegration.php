...
final class RestApiIntegration {
    ...
    public function ajax_save_rate_limits(): void {
        ...
        if ( ! in_array( $unit, [ 'minute', 'hour', 'day' ], true ) ) {
            wp_send_json_error( [ 'message' => __( 'Invalid rate limit unit.', Config::TEXTDOMAIN ) ] );
        }
        ...
        if ( $updated ) {
            wp_send_json_success([
                'message' => $unchanged
                    ? sprintf( __( 'Rate limit %s unchanged (%d).', Config::TEXTDOMAIN ), $label, $value )
                    : sprintf( __( 'Rate limit %s updated to %d.', Config::TEXTDOMAIN ), $label, $value ),
                'unit' => $unit,
                'value' => $value,
            ]);
        } else {
            wp_send_json_error([
                'message' => __( 'Failed to save rate limit.', Config::TEXTDOMAIN ),
            ]);
        }
    }
    ...
    public function ajax_toggle_restapi_service(): void {
        ...
        if ( ! $enable && ! $force_disable ) {
            wp_send_json_error([
                'message' => $attachment_enabled 
                    ? __( 'File attachment is currently enabled and requires the REST API service.', Config::TEXTDOMAIN )
                    : __( 'The REST API service is required for file attachment uploads. Disabling it will prevent the file upload feature from working.', Config::TEXTDOMAIN ),
        ...
        $button = $enable
            ? __( 'Disable REST API Service', Config::TEXTDOMAIN )
            : __( 'Enable REST API Service', Config::TEXTDOMAIN );
        ...
        $message = $enable 
            ? __( 'REST API service enabled successfully', Config::TEXTDOMAIN )
            : ( $attachment_was_disabled 
                ? __( 'REST API service and file attachment feature disabled successfully', Config::TEXTDOMAIN )
                : __( 'REST API service disabled successfully', Config::TEXTDOMAIN )
            );
    ...
    public function ajax_generate_token(): void {
        ...
        try {
            ...
            wp_send_json_success( [
                'message' => __( 'Token generated successfully. Copy it now – you won\'t see it again!', Config::TEXTDOMAIN ),
    ...
    public function ajax_revoke_token(): void {
        ...
        if ( empty( $token_id ) ) {
            wp_send_json_error( [ 'message' => __( 'Invalid token ID.', Config::TEXTDOMAIN ) ] );
        }
        ...
        try {
            ...
            if ( $revoked ) {
                wp_send_json_success( [ 'message' => __( 'Token revoked successfully.', Config::TEXTDOMAIN ) ] );
            } else {
                wp_send_json_error( [ 'message' => __( 'Token not found or already revoked.', Config::TEXTDOMAIN ) ] );
            }
        } catch ( Exception $e ) {
            wp_send_json_error( [ 'message' => $e->getMessage() ] );
        }
    ...
    public function ajax_test_connection(): void {
        ...
        if ( empty( $name ) || empty( $email ) || empty( $message ) ) {
            wp_send_json_error( [ 'message' => __( 'All fields are required.', Config::TEXTDOMAIN ) ] );
        }

        if ( ! is_email( $email ) ) {
            wp_send_json_error( [ 'message' => __( 'Invalid email address.', Config::TEXTDOMAIN ) ] );
        }
        ...
        if ( $has_attachment ) {
            if ( ! empty( $_FILES['attachment']['error'] ) ) {
                wp_send_json_error( [
                    'message' => __( 'Attachment upload failed.', Config::TEXTDOMAIN ),
                    'error'   => 'upload_error',
                ] );
            }
        ...
        if ( is_wp_error( $response ) ) {
            wp_send_json_error( [
                'message' => sprintf(
                    __( 'Connection failed: %s', Config::TEXTDOMAIN ),
        ...
        if ( $status_code >= 200 && $status_code < 300 ) {
            wp_send_json_success( [
                'message'     => __( 'Test request successful!', Config::TEXTDOMAIN ),
            ...
            wp_send_json_error( [
                'message'     => __( 'Test request failed with status code: ', Config::TEXTDOMAIN ) . $status_code,
                'status_code' => $status_code,
                'response'    => $data,
                'error'       => $data['message'] ?? __( 'Unknown error', Config::TEXTDOMAIN ),
            ] );
        }
    }
}
