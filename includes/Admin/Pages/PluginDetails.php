...
final class PluginDetails {
	...
	public function render_details(): void {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			@ini_set( 'display_errors', 0 ); // phpcs:ignore WordPress.PHP.IniSet.display_errors_Disallowed, WordPress.PHP.NoSilencedErrors.Discouraged, Squiz.PHP.DiscouragedFunctions.Discouraged -- Intentional: suppress PHP errors in AJAX output for clean JSON response.
		}
		$template = $this->get_template_path( 'modal' );

		echo TemplateLoader::render(
			$template,
			array(
