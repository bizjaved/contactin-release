...
final class CRMLog {
    ...
    public function ajax_download_csv(): void {
        ...
        if (!$valid_nonce) {
            wp_send_json_error(['message' => __('Security check failed.',  'contactin')], 403);
        }
        ...
        if (empty($rows)) {
            wp_send_json_error([ 'message' => __( 'No logs to export.',  'contactin') ]);
        }
    ...
    public function ajax_prune_old_logs(): void {
        ...
        if ($count > 0) {
            wp_send_json_success(['message' => sprintf(__('Pruned %d old CRM logs.',  'contactin'), $count)]);
        } else {
            wp_send_json_success(['message' => __('No old CRM logs to prune.',  'contactin')]);
        }
    }
    ...
    public function ajax_clear_all_logs(): void {
        ...
        if ($count > 0 || $count === 0) { // 0 is also valid (table was empty or truncated)
            wp_send_json_success(['message' => __('All CRM logs cleared successfully.',  'contactin')]);
        } else {
            wp_send_json_error(['message' => __('Failed to clear CRM logs.',  'contactin')]);
        }
    }
    ...
    private function prepare_export_rows(array $rows): array {
        foreach ($rows as &$row) {
            ...
            if (!in_array($operation, ['contact_delete', 'crm_delete'], true)) {
                continue;
            }
