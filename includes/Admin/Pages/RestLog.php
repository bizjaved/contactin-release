...
final class RestLog {
	...
	private function __construct() {
		...
		// Modern naming
		add_action( 'wp_ajax_contactinbox_view_rest_log', array( $this, 'ajax_view' ) );
		add_action( 'wp_ajax_contactinbox_get_adjacent_rest_log', array( $this, 'ajax_get_adjacent_rest_log' ) );
		add_action( 'wp_ajax_contactinbox_prune_rest', array( $this, 'ajax_prune' ) );
		add_action( 'wp_ajax_contactinbox_clear_rest_logs', array( $this, 'ajax_clear_all' ) );
		add_action( 'wp_ajax_contactinbox_download_rest_csv', array( $this, 'ajax_download_csv' ) );
		add_action( 'wp_ajax_contactinbox_rest_export_info', array( $this, 'ajax_export_info' ) );

		// Legacy support (JavaScript uses these)
		add_action( 'wp_ajax_contactin_view_rest_log', array( $this, 'ajax_view' ) );
		add_action( 'wp_ajax_contactin_get_adjacent_rest_log', array( $this, 'ajax_get_adjacent_rest_log' ) );
		add_action( 'wp_ajax_contactin_prune_rest', array( $this, 'ajax_prune' ) );
		add_action( 'wp_ajax_contactin_clear_rest_logs', array( $this, 'ajax_clear_all' ) );
		add_action( 'wp_ajax_contactin_download_rest_csv', array( $this, 'ajax_download_csv' ) );
	}
	...
	public function ajax_view(): void {
		...
		if ( ! $row ) {
			wp_send_json_error( array( 'message' => __( 'Log entry not found.', 'contactin' ) ) );
		}
	...
	public function ajax_prune(): void {
		...
		if ( $deleted > 0 ) {
			...
			wp_send_json_success( array( 'message' => __( 'No old REST API logs to prune.', 'contactin' ) ) );
		}
	}
	...
	public function ajax_clear_all(): void {
		...
		if ( $this->repo->clear_all() ) {
			...
			wp_send_json_error( array( 'message' => __( 'Failed to clear REST API logs.', 'contactin' ) ) );
		}
	}

	public function ajax_download_csv(): void {
			// CSV export is available in this package.
