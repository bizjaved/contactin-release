...
final class RestLog {
    ...
    public static function render(): void {
        ...
        if ( file_exists( $template ) ) {
            ...
            echo '<div class="notice notice-error"><p>'
                . esc_html__( 'REST API log template not found.', Config::TEXTDOMAIN )
                . '</p></div>';
        }
    }
    ...
    public function ajax_view(): void {
        ...
        if ( ! $row ) {
            wp_send_json_error([ 'message' => __( 'Log entry not found.', Config::TEXTDOMAIN ) ]);
        }
    ...
    public function ajax_prune(): void {
        ...
        if ( $deleted > 0 ) {
            wp_send_json_success([
                'message' => sprintf(
                    __( 'Pruned %d REST logs older than %d days.', Config::TEXTDOMAIN ),
                    (int) $deleted,
                    $retention_days
                ),
                'deleted' => (int) $deleted,
            ...
            wp_send_json_error([ 'message' => __( 'Failed to prune REST API logs.', Config::TEXTDOMAIN ) ]);
        }
    }
    ...
    public function ajax_clear_all(): void {
        ...
        if ( $this->repo->clear_all() ) {
            wp_send_json_success([
                'message' => __( 'All REST API logs have been cleared successfully.', Config::TEXTDOMAIN ),
            ]);
        } else {
            wp_send_json_error([ 'message' => __( 'Failed to clear REST API logs.', Config::TEXTDOMAIN ) ]);
        }
    }
    ...
    public function ajax_download_csv(): void {
            ...
            if (empty($rows)) {
                wp_send_json_error([
                    'message' => __( 'No logs to export.', Config::TEXTDOMAIN ),
                ]);
            }
    ...
    public function ajax_export_info(): void {
            ...
            wp_send_json_success([
                ...
                'limit'   => $limit,
                'batches' => $batches,
                'message' => sprintf( __( 'Found %d REST API logs. Export limit: %d per file.', Config::TEXTDOMAIN ), $total, $limit ),
            ]);
    }
