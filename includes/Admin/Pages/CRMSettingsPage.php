...
final class CRMSettingsPage {
    ...
    public function ajax_test_connection(): void {
        ...
        $nonce = $_POST['nonce'] ?? '';
        if (!wp_verify_nonce($nonce, Config::CRM_SETTINGS_NONCE_ACTION)) {
            wp_send_json_error(['message' => __('Security check failed (nonce).',  'contactin')]);
        }
        ...
        if (empty($endpoint)) {
            wp_send_json_error(['message' => __('Endpoint is required.',  'contactin')]);
        }
        ...
        try {
            ...
            if (is_wp_error($test_response)) {
                wp_send_json_error(['message' => __('Connection failed: ',  'contactin') . $test_response->get_error_message()]);
            }

            $status_code = wp_remote_retrieve_response_code($test_response);
            if ($status_code >= 200 && $status_code < 400) {
                wp_send_json_success(['message' => sprintf(__('Connection successful (HTTP %d)',  'contactin'), $status_code)]);
            } else {
                $response_body = wp_remote_retrieve_body($test_response);
                wp_send_json_error(['message' => sprintf(__('Connection failed (HTTP %d) - Check your credentials',  'contactin'), $status_code)]);
            }
        } catch (\Exception $e) {
            wp_send_json_error(['message' => __('Test failed: ',  'contactin') . $e->getMessage()]);
        }
    }
    ...
    public function ajax_save_oauth_credentials(): void {
        ...
        if (!current_user_can(Config::CAPABILITY)) {
            wp_send_json_error(['message' => __('Unauthorized',  'contactin')]);
        }
        ...
        if (empty($consumer_key)) {
            wp_send_json_error(['message' => __('Consumer Key is required',  'contactin')]);
        }
        ...
        if (empty($oauth_url)) {
            wp_send_json_error(['message' => __('Failed to generate OAuth URL. Please check your credentials.',  'contactin')]);
        }
    ...
    public function ajax_disconnect_crm(): void {
        ...
        wp_send_json_success(['message' => __('Disconnected from Salesforce successfully.',  'contactin')]);
    }
}
