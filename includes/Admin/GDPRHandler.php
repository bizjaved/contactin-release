...
final class GDPRHandler {
    ...
    public function handle_delete_contact(): void {
                ...
                if ($file_path && file_exists($file_path)) {
                    if (@unlink($file_path)) {
                        ...
                        $deletion_errors[] = sprintf(
                            __('Failed to delete file: %s', Config::TEXTDOMAIN),
                            basename($file_path)
                        );
                    }
                }
        ...
        if ($contact_deleted !== false) {
            wp_send_json_success([
                'message' => __('Contact and all associated data deleted successfully.', Config::TEXTDOMAIN),
                'messages_deleted' => $messages_deleted,
                'files_deleted' => $files_deleted,
                'crm_sync_status' => $crm_sync_status,
            ]);
    ...
    public function handle_delete_from_crm(): void {
        ...
        $logs = $wpdb->get_results(
            "SELECT id, email, name FROM {$table_gdpr_log} 
        ...
        // CRM deletion is handled through the queue-based crm_delete queue type
        // See CronJobs::process_crm_queue() and CronJobs::process_crm_delete()
        foreach ($logs as $log) {
            // Queue item is created automatically when deletion is marked for CRM sync
            // The unified queue processor (CronJobs::process_crm_delete) handles the deletion
            
            // For now, just log this would be queued for deletion
            if (defined('WP_DEBUG') && WP_DEBUG) {
                Logger::notice('Contact queued for CRM deletion', [
                    'email' => $log->email,
                    'name' => $log->name,
                    'log_id' => $log->id,
                ]);
        ...
        wp_send_json_success([
            'message' => sprintf(
                __('%d contacts marked for CRM deletion.', Config::TEXTDOMAIN),
                $deleted_count
            ),
            'deleted_count' => $deleted_count,
            'note' => __('CRM deletion requires additional integration setup.', Config::TEXTDOMAIN),
        ]);
    }
