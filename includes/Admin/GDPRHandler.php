...
final class GDPRHandler {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_ci_gdpr_link', array( $this, 'handle_link' ) );
		add_action( 'wp_ajax_ci_gdpr_send_email', array( $this, 'handle_send_email' ) );
		add_action( 'wp_ajax_ci_gdpr_delete', array( $this, 'handle_delete' ) );
		add_action( 'wp_ajax_ci_gdpr_delete_contact', array( $this, 'handle_delete_contact' ) );
		add_action( 'wp_ajax_ci_gdpr_delete_from_crm', array( $this, 'handle_delete_from_crm' ) );
		add_action( 'contactinbox_send_gdpr_email_event', array( $this, 'send_gdpr_email_async' ), 10, 3 );
	}
	...
	public function handle_link(): void {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], Config::GDPR_NONCE_ACTION ) ) {
			wp_send_json_error( array( 'message' => Config::GDPR_MSG_SECURITY_FAIL ) );
		}
		if ( ! current_user_can( Config::CAPABILITY ) ) {
			wp_send_json_error( array( 'message' => Config::GDPR_MSG_PERMISSION ) );
		}
		...
		if ( ! $message || empty( $message->email ) ) {
			wp_send_json_error( array( 'message' => Config::GDPR_MSG_NO_DATA ) );
		}
		...
		if ( ! $token ) {
			wp_send_json_error( array( 'message' => Config::GDPR_MSG_LINKGEN_FAIL ) );
		}
	...
	public function send_gdpr_email_async( string $email, string $deletion_link, array $message_data = array() ): void {
		try {
			SMTP::send_gdpr_email( $email, $deletion_link, $message_data );
		} catch ( \Throwable $e ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				Logger::error(
					'GDPR email send failed',
					array(
						'error' => $e->getMessage(),
						'email' => $email,
					)
	...
	public function handle_delete_contact(): void {
		...
		foreach ( $messages as $msg ) {
			if ( ! empty( $msg->attachment ) ) {
				...
				if ( $file_path && file_exists( $file_path ) ) {
					if ( @unlink( $file_path ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged -- Intentional: unlink() warnings suppressed; is_file()/is_writable() checked above.
						++$files_deleted;
					} else {
		...
		$deleted_count = $wpdb->delete( $table_messages, array( 'contact_id' => $contact_id ), array( '%d' ) );
		if ( $deleted_count !== false ) {
			$messages_deleted = $deleted_count;
		}

		// Delete the contact record
		$contact_deleted = $wpdb->delete( $table_contacts, array( 'id' => $contact_id ), array( '%d' ) );
		...
		foreach ( $logs as $log ) {
			try {
				...
				if ( ! is_wp_error( $queue_id ) ) {
					...
					$wpdb->update(
						$table_gdpr_log,
						array( 'crm_deletion_queued_at' => current_time( 'mysql' ) ),
						array( 'id' => $log->id ),
						array( '%s' ),
						array( '%d' )
					);
