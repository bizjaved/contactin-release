...
final class SupportBoxesManager {
    ...
    public static function render_support_boxes( string $context = '' ): void {
        ...
        $boxes = array_values( array_diff( $boxes, [ 'expired-license' ] ) );
    ...
    private static function maybe_handle_feedback_box_action(): void {
        ...
        if ( ! in_array( $action, [ 'snooze', 'dismiss' ], true ) || ! self::is_feedback_box_id_allowed( $box_id ) ) {
            return;
        }
        ...
        $redirect_url = remove_query_arg(
            [ self::PRO_FEEDBACK_ACTION_PARAM, self::PRO_FEEDBACK_NONCE_PARAM, self::FEEDBACK_BOX_PARAM ],
            $current_url
        );
    ...
    public static function get_box_action_url( string $box_id, string $action ): string {
        if ( ! self::is_feedback_box_id_allowed( $box_id ) || ! in_array( $action, [ 'snooze', 'dismiss' ], true ) ) {
            return '';
        }
    ...
    private static function is_feedback_box_id_allowed( string $box_id ): bool {
        return in_array( $box_id, [ 'pro-feedback', 'wordpress-review', 'github-feedback' ], true );
    }
