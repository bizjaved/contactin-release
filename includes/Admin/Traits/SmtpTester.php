...
trait SmtpTester {
    public function ajax_test_smtp(): void {
        ...
        if (!current_user_can(Config::CAPABILITY)) {
            wp_send_json_error(['code' => 'permission_denied', 'message' => __('Permission denied.', Config::TEXTDOMAIN)]);
        }

        $to = sanitize_email($_POST['to'] ?? $_POST['admin_email'] ?? get_option('admin_email'));
        if (!is_email($to)) {
            wp_send_json_error(['code' => 'invalid_email', 'message' => __('Invalid test email address.', Config::TEXTDOMAIN)]);
        }
        ...
        if ($port < 1 || $port > 65535) {
            wp_send_json_error(['code' => 'invalid_port', 'message' => __('Invalid SMTP port.', Config::TEXTDOMAIN)]);
        }
    ...
    public function ajax_check_smtp_result(): void {
        ...
        if ($key === '') {
            wp_send_json_error(['code' => 'missing_key', 'message' => __('Missing key.', Config::TEXTDOMAIN)]);
        }

        if (!apply_filters('contactinbox_allow_smtp_poll', true, $key)) {
            wp_send_json_error(['code' => 'poll_limited', 'message' => __('Polling limited. Please wait a moment.', Config::TEXTDOMAIN)]);
        }
        ...
        if ($result) {
            ...
            wp_send_json_success([
                ...
                'message' => __('SMTP result available.', Config::TEXTDOMAIN),
                'result'  => $result,
            ]);
        }

        wp_send_json_error(['code' => 'smtp_result_pending', 'message' => __('No result yet.', Config::TEXTDOMAIN)]);
    }
}
