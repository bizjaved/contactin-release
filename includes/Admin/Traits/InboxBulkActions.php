...
trait InboxBulkActions {
	...
	public function ci_bulk_action(): void {
		...
		if ( empty( $ids ) ) {
			wp_send_json_error( array( 'message' => __( 'No messages selected.', 'contactin' ) ) );
		}

		if ( ! in_array( $action, array( 'read', 'unread', 'archive', 'unarchive', 'delete', 'spam', 'not_spam' ), true ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid bulk action.', 'contactin' ) ) );
		}
		...
		if ( ! $count ) {
			wp_send_json_error( array( 'message' => __( 'No changes made.', 'contactin' ) ) );
		}
	...
	public function ci_clear_spam(): void {
		...
		if ( ! $count ) {
			wp_send_json_error( array( 'message' => __( 'No spam messages to delete.', 'contactin' ) ) );
		}
	...
	public function ci_clear_archives(): void {
		...
		if ( ! $count ) {
			wp_send_json_error( array( 'message' => __( 'No archived messages to delete.', 'contactin' ) ) );
		}
	...
	private function cleanup_attachments_for_ids( array $ids ): array {
		...
		if ( ! empty( $deletion_results['failed'] ) ) {
			\ContactInbox\Core\Logger::warning(
				'Some attachment files could not be deleted during bulk deletion',
				array(
					'message_ids'  => $ids,
