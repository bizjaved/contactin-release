...
trait ContactEditAjaxHandler {
    ...
    public function handle_update_contact(): void {
        ...
        if (empty($update_data)) {
            wp_send_json_error(['message' => __('No data to update.', Config::TEXTDOMAIN)]);
        }
        ...
        if ($updated) {
            wp_send_json_success([
                'message' => __('Contact updated successfully.', Config::TEXTDOMAIN),
            ]);
        } else {
            wp_send_json_error([
                'message' => __('Failed to update contact.', Config::TEXTDOMAIN),
            ]);
        }
    }
