...
trait InboxPageRenderer {
	...
	public function display_page(): void {
		...
		if ( file_exists( $template ) ) {
			...
			wp_die( esc_html__( 'Inbox template not found.', 'contactin' ) );
		}
	...
	private function sanitize_inbox_filters(): array {
		...
		$allowed_intents = array( 'all', 'sales', 'support', 'feedback', 'complaint', 'question', 'spam', 'unclassified' );
		if ( ! in_array( $intent, $allowed_intents, true ) ) {
			$intent = 'all';
		}

		// Validate orderby against allowed columns
		$allowed_columns = array( 'id', 'name', 'email', 'subject', 'message', 'status', 'submitted_at' );
