...
class AlertSystem {
	...
	public static function get_unread_count(): int {
		try {
			...
			Logger::error( 'Failed to get unread count', array( 'error' => $e->getMessage() ) );
			return 0;
		}
	}
	...
	public static function mark_as_read( int $alert_id ): void {
		try {
			global $wpdb;
			$table = $wpdb->prefix . self::ALERT_TABLE;
			$wpdb->update( $table, array( 'is_read' => 1 ), array( 'id' => $alert_id ), array( '%d' ), array( '%d' ) );
		} catch ( \Throwable $e ) {
			Logger::error(
	...
	private static function send_admin_email( string $type, string $message, string $level ): void {
		try {
			...
			Logger::error(
				'Failed to send admin email alert',
				array(
					'error' => $e->getMessage(),
				)
			);
