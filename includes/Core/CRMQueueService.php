...
final class CRMQueueService {
    ...
    public static function queue_message_sync(int $message_id, int $priority = 3, bool $reset_status = false): bool {
        if ($message_id <= 0) {
            Logger::warning('CRM queue attempted with invalid message_id', ['message_id' => $message_id]);
            return false;
        }

        $crm_settings = CRMSettings::get_settings();
        if (empty($crm_settings['crm_enabled'])) {
            Logger::debug('CRM sync skipped - CRM not enabled', ['message_id' => $message_id]);
            return false;
        }

        $payload = CRMQueuePayloadBuilder::build_from_message_id($message_id);
        if (!$payload) {
            Logger::warning('CRM queue payload build failed', [
        ...
        if (!empty($payload['attachment'])) {
            Logger::debug('CRM queue payload includes attachment', [
                'message_id' => $message_id,
            ...
            Logger::debug('CRM queue payload - no attachment', ['message_id' => $message_id]);
        }
        ...
        Logger::info('CRM sync queued successfully', [
            'message_id' => $message_id,
            'queue_id' => $queue_id,
            'priority' => $priority,
