...
final class CRMAuth {
	...
	/**
	 * Get Salesforce authentication tokens and instance URL.
	 *
	 * @param array $settings CRM settings array (optional, will fetch if not provided).
	 * @param bool  $force_refresh When true, bypasses cache and refreshes token.
	 * @return array|WP_Error Array with 'access_token' and 'instance_url' keys, or WP_Error on failure.
	 */
	public static function get_auth_tokens( array $settings = array(), bool $force_refresh = false ) {
		if ( empty( $settings ) ) {
			$settings = CRMSettings::get_settings();
		}

		$access_token = self::get_access_token( $force_refresh );
		if ( is_wp_error( $access_token ) ) {
			return $access_token;
		}

		$instance_url = trim( (string) ( $settings['instance_url'] ?? '' ) );
		if ( $instance_url === '' ) {
			return new WP_Error( 'crm_no_instance_url', __( 'Salesforce instance URL not configured. Please reconnect to Salesforce.', 'contactin' ) );
		}
	...
	private static function get_oauth_token( bool $force_refresh = false ) {
		...
		if ( $force_refresh || $access_token === '' ) {
			if ( $refresh_token === '' ) {
				return new WP_Error( 'crm_oauth_no_refresh_token', __( 'No refresh token available. Please reconnect to Salesforce.', 'contactin' ) );
			}
	...
	private static function refresh_oauth_token( string $refresh_token, array $settings ) {
		...
		if ( $client_id === '' ) {
			return new WP_Error( 'crm_oauth_missing_client', __( 'Salesforce connected app client ID missing. Please reconnect to Salesforce.', 'contactin' ) );
		}
		...
		if ( $code < 200 || $code >= 300 || empty( $body['access_token'] ) ) {
			...
			return new WP_Error( 'crm_oauth_refresh_error', sprintf( __( 'Failed to refresh token (HTTP %d): %s', 'contactin' ), $code, $detail ) );
		}
