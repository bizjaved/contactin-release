...
final class CRMQueuePayloadBuilder {
	...
	public static function build_from_message( Message $message ): array {
		...
		if ( ! empty( $message->attachment ) ) {
			Logger::info(
				'CRM payload: extracting attachment from message',
				array(
					'message_id'         => $message->id,
			...
			Logger::info(
				'CRM payload: attachment paths extracted',
				array(
					'message_id'  => $message->id,
			...
			foreach ( $file_paths as $file_path ) {
				if ( $file_path && AttachmentHelper::is_valid_file( $file_path ) ) {
					...
					Logger::debug(
						'CRM payload: attachment path validated',
						array(
							'message_id'  => $message->id,
							'path'        => $file_path,
							'file_exists' => true,
					...
					Logger::warning(
						'CRM payload: attachment path validation failed',
						array(
							'message_id'  => $message->id,
							'path'        => $file_path,
			...
			if ( ! empty( $valid_paths ) ) {
				...
				Logger::info(
					'CRM payload: attachment included',
					array(
						'message_id' => $message->id,
				...
				if ( ! empty( $file_paths ) ) {
					...
					Logger::warning(
						'CRM payload: extracted attachment paths but files not valid',
						array(
							'message_id'      => $message->id,
					...
					Logger::warning(
						'CRM payload: attachment extraction returned empty',
						array(
							'message_id'             => $message->id,
			...
			Logger::debug(
				'CRM payload: no attachment in message',
				array(
					'message_id' => $message->id,
				)
			);
