...
final class SMTP {
	...
	private function __construct() {
		...
		add_action( \ContactInbox\Core\Config::CRON_SMTP_TEST, array( $this, 'handle_smtp_test' ), 10, 2 );

		// Email notification hooks
		add_action( 'contactin_send_admin_notification', array( self::class, 'send_admin_notification' ) );
		add_action( 'contactin_send_user_confirmation', array( self::class, 'send_user_confirmation' ) );
	}
	...
	private static function apply_smtp_settings( PHPMailer $mailer, array $settings ): void {
		...
		if ( $from_email === '' && $username !== '' ) {
			...
			if ( $fallback_email !== '' ) {
				...
				Logger::warning(
					'SMTP sender email missing; falling back to SMTP username',
					array(
						'smtp_user' => $username,
					)
				);
		...
		if ( $from_email !== '' && $username !== '' ) {
			...
			if ( $from_domain !== '' && $user_domain !== '' && $from_domain !== $user_domain ) {
				Logger::warning(
					'SMTP sender domain differs from SMTP username domain',
					array(
	...
	public static function send(
		...
		if ( empty( $recipients ) ) {
			...
			Logger::warning(
				'SMTP send aborted: no valid recipient',
				array(
		...
		if ( empty( $settings['smtp_enable'] ) ) {
			...
			Logger::log(
				$result ? Logger::INFO : Logger::ERROR,
				'wp_mail dispatch result',
				array(
					'recipients' => $recipient_list,
					'subject'    => $subject,
					'context'    => $context['type'] ?? EmailLog::TYPE_CONTACT_FORM,
	...
	private static function render_email( string $template, array $vars = array() ): string {
	...
	public static function send_user_confirmation( array $data, string $subject = '' ): bool {
		...
		$settings_form   = get_option( Config::OPTION_SETTINGS, array() );
		...
		return self::send(
			...
			$body,
			$headers,
			array( 'type' => EmailLog::TYPE_USER_CONFIRM )
		);
	}
	...
	// SMTP Test
	// =========================================================================
	public static function test_smtp( string $to = '', array $override_settings = array(), bool $suppress_log = false ): array {
		self::ensure_phpmailer_loaded();
		$settings                = array_merge( get_option( Config::OPTION_SETTINGS, array() ), $override_settings );
		...
		$body = self::render_email( Config::TEMPLATE_EMAIL . 'smtp-test.php', array() );
		if ( $body === '' ) {
	 ...
	 */
	public static function send_gdpr_email( string $to, string $delete_link, array $message_data = array() ): bool {
		...
		$result = self::send( $recipient_list, $subject, $body, $headers, array( 'type' => EmailLog::TYPE_GDPR ) );
