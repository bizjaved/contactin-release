...
final class ErrorClassifier {
	 ...
	 */
	private const NON_RETRIABLE = array(
		self::AUTH,
		self::VALIDATION,
		self::FIELD_MAPPING,
	);
	...
	public static function classify_by_exception( \Throwable $exception ): string {
		...
		// Auth errors
		if ( self::contains_any( $message, array( 'unauthorized', 'invalid token', 'expired', 'forbidden', '401', '403' ) ) ) {
			return self::AUTH;
		}

		// Validation errors
		if ( self::contains_any( $message, array( 'required', 'invalid', 'missing field', 'validation', 'bad request', '400' ) ) ) {
			return self::VALIDATION;
		}

		// Field mapping errors (data structure issues)
		if ( self::contains_any( $message, array( 'field mapping', 'no such column', 'unknown attribute', 'field does not exist' ) ) ) {
			return self::FIELD_MAPPING;
		}

		// Rate limiting
		if ( self::contains_any( $message, array( 'rate limit', 'too many requests', '429', 'throttle' ) ) ) {
			return self::RATE_LIMIT;
		}

		// Timeout errors
		if ( self::contains_any( $message, array( 'timeout', 'time out', 'timed out', 'connection timeout', '408', '504' ) ) ) {
			return self::TIMEOUT;
		}

		// Server errors
		if ( self::contains_any( $message, array( 'server error', '500', '502', '503', 'service unavailable', 'bad gateway' ) ) ) {
			return self::SERVER_ERROR;
		}

		// Network errors
		if ( self::contains_any( $message, array( 'connection failed', 'network error', 'dns', 'unreachable', 'refused' ) ) ) {
