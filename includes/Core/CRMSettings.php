...
final class CRMSettings
{
    ...
    public static function sanitize(array $settings): array
    {
        $sanitized = [
            ...
            'environment'      => in_array($settings['environment'] ?? 'production', ['production', 'sandbox'], true)
            ...
            'name_field_order' => in_array($settings['name_field_order'] ?? 'first_last', ['first_last', 'last_first'], true) 
        ...
        $sanitized['phone_handling_strategy'] = in_array(
            ...
            ['overwrite', 'secondary', 'skip_if_exists'],
            true
        ) ? $settings['phone_handling_strategy'] : 'secondary';
    ...
    {
        return get_option(Config::OPTION_CRM, []);
    }
