...
final class ContactResolver
{
    ...
    public static function resolve(array $payload, string $default_country = '1'): array
    {
        ...
        if ($email !== '') {
            ...
            if ($contact) {
                ...
                if (!empty($salutation) && empty($contact->salutation)) {
                    $repo->update($contact->id, ['salutation' => $salutation]);
                }
                return ['contact_id' => $contact->id, 'phones' => $phones];
            }
        }
        ...
        foreach (['mobile_phone', 'phone', 'home_phone', 'other_phone'] as $key) {
            if (!empty($phones[$key])) {
                ...
                if ($contact) {
                    ...
                    if ($email !== '' && empty($contact->email)) {
                        $repo->update($contact->id, ['email' => $email]);
                    }
                    return ['contact_id' => $contact->id, 'phones' => $phones];
                }
            }
        ...
        if ($email !== '') {
            ...
            if ($existing) {
                ...
                return ['contact_id' => $existing->id, 'phones' => $phones];
            }
        }
        ...
        return ['contact_id' => $new_id ?: null, 'phones' => $phones];
    }
    ...
    private static function merge_contact_phones($contact, array $incoming, ContactRepository $repo, string $default_country): void {
        ...
        $preferences = [
            'mobile_phone' => ['mobile_phone', 'primary_phone', 'home_phone', 'other_phone'],
            'phone'        => ['primary_phone', 'mobile_phone', 'home_phone', 'other_phone'],
            'home_phone'   => ['home_phone', 'primary_phone', 'mobile_phone', 'other_phone'],
            'other_phone'  => ['other_phone', 'home_phone', 'mobile_phone', 'primary_phone'],
        ];
