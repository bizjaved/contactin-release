...
final class DatabaseOptimizer {
	...
	public static function get_innodb_status(): array {
		...
		try {
			...
			if ( ! $status ) {
				return array( 'error' => 'InnoDB engine not available' );
			}
	...
	public static function optimize_table(): bool {
		...
		try {
			...
			Logger::error( 'Table optimization failed', array( 'error' => $e->getMessage() ) );
			return false;
		}
	}
	...
	public static function kill_long_transactions( int $timeout_seconds = 300 ): int {
		...
		try {
			...
			Logger::error( 'Failed to kill transactions', array( 'error' => $e->getMessage() ) );
			return 0;
		}
	}
