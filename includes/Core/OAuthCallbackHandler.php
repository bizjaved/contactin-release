...
final class OAuthCallbackHandler {
    ...
    public static function handle_callback(): void {
        ...
        if (!$stored_state) {
            ...
            wp_die(__('OAuth state not found. Session may have expired. Please try again.', Config::TEXTDOMAIN));
        }

        if (!hash_equals($state, (string) $stored_state)) {
            Logger::warning('OAuth state mismatch detected');
            wp_die(__('OAuth state mismatch - possible CSRF attack. Please try again.', Config::TEXTDOMAIN));
        }
        ...
        if (!$code) {
            ...
            Logger::error(
                ...
                [ 'error' => $error, 'description' => $error_desc ]
            );
            wp_die(__('Authorization failed: ', Config::TEXTDOMAIN) . esc_html($error) . ' - ' . esc_html($error_desc));
        }
        ...
        if (!$code_verifier) {
            ...
            wp_die(__('PKCE code verifier expired. Please try again.', Config::TEXTDOMAIN));
        }
        ...
        if (empty($client_id)) {
            ...
            wp_die(__('OAuth Consumer Key not configured. Please configure Salesforce OAuth settings.', Config::TEXTDOMAIN));
        }
        ...
        if (is_wp_error($response)) {
            Logger::error(
                ...
                [ 'error' => $response->get_error_message(), 'endpoint' => $token_url ]
            );
            wp_die(__('Token exchange failed: ', Config::TEXTDOMAIN) . esc_html($response->get_error_message()));
        }
        ...
        if (!isset($body['access_token'])) {
            ...
            wp_die(__('No access token received from Salesforce.', Config::TEXTDOMAIN));
        }
        ...
        Logger::info('OAuth endpoint configured', [ 'endpoint' => $settings['endpoint'] ]);
