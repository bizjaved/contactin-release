<?php
// phpcs:disable WordPress.WP.I18n.UnorderedPlaceholdersText, WordPress.WP.I18n.MissingTranslatorsComment
/**
 * ContactIn – CRM Monitoring & Statistics
 *
...
final class CRMMonitor {
	...
	private static function format_endpoint_display( string $endpoint ): string {
		if ( stripos( $endpoint, '/sobjects/Contact/Email/' ) !== false ) {
			return __( 'Contact Upsert (Email)', 'contactin' );
		}

		if ( stripos( $endpoint, '/sobjects/Case' ) !== false ) {
			return __( 'Case Create', 'contactin' );
		}
	...
	private static function get_health_message( string $status, float $success_rate, int $total ): string {
		if ( $total === 0 ) {
			return __( 'No CRM activity recorded yet.', 'contactin' );
		}

		switch ( $status ) {
			case 'critical':
				return sprintf(
					__( 'Critical: Only %.1f%% of CRM sends are succeeding. Check endpoints and authentication.', 'contactin' ),
					$success_rate
				);
			case 'warning':
				return sprintf(
					__( 'Warning: CRM success rate is %.1f%%. Monitor for issues.', 'contactin' ),
					$success_rate
				);
			case 'healthy':
				return sprintf(
					__( 'Healthy: CRM integration operating normally (%.1f%% success rate)', 'contactin' ),
					$success_rate
				);
			default:
				return __( 'CRM status is unknown. Awaiting activity.', 'contactin' );
		}
	}
}
