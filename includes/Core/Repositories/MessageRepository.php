...
final class MessageRepository {
	...
	public function insert( array $data ): int|false {
		...
		$format = array( '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%f', '%d' );
	...
	public function get_by_contact(
		...
		$offset          = max( 0, ( $page - 1 ) * $per_page );
		$order           = strtoupper( $order ) === 'ASC' ? 'ASC' : 'DESC';
		$allowed_orderby = array( 'submitted_at', 'status', 'subject' );
		...
		return empty( $rows ) ? array() : array_map( fn( $row ) => new Message( $row ), $rows );
	}
	...
	public function get_ids_and_attachments_by_email( string $email ): array {
		...
		return $wpdb->get_results( $query, OBJECT ) ?: array();
	}
	...
	public function delete( int $id ): bool {
		...
		return (bool) $wpdb->delete( $this->table_messages, array( 'id' => $id ), array( '%d' ) );
	}
	...
	public function bulk_update_status( array $ids, string $status ): int {
		...
		if ( ! in_array( $status, array( Config::STATUS_UNREAD, Config::STATUS_READ ), true ) ) {
			return 0;
		}
	...
	public function update_archive( int $message_id, bool $archived ): bool {
		...
		$result = $wpdb->update(
			$this->table_messages,
			array( 'is_archived' => (int) $archived ),
			array( 'id' => $message_id ),
			array( '%d' ),
			array( '%d' )
		);
	...
	private function validate_column_name( string $column ): string {
		$allowed = array( 'id', 'name', 'email', 'subject', 'message', 'status', 'submitted_at' );
		return in_array( $column, $allowed, true ) ? $column : 'submitted_at';
	}
		...
		foreach ( $results as $row ) {
			...
			if ( $has_stale ) {
				if ( $data === null ) {
					...
					$wpdb->update(
						$this->table_messages,
						array( 'attachment' => wp_json_encode( $data ) ),
						array( 'id' => $row->id ),
						array( '%s' ),
						array( '%d' )
					);
	...
	public function mark_message_as_spam( int $message_id ): bool {
		...
		$result = $wpdb->update(
			...
			array(
				...
				'intent_keywords'      => wp_json_encode( array( 'manual' ) ),
	...
	public function get_inbox_status_breakdown( ?string $start_date = null, ?string $end_date = null ): array {
		...
		$where_clauses = array( '(recaptcha_score IS NULL OR recaptcha_score >= %f)' );
		$where_params  = array( Config::SPAM_SCORE_THRESHOLD );
		...
		$params = array_merge(
			array( Config::STATUS_UNREAD, Config::STATUS_READ ),
			$where_params
		);
