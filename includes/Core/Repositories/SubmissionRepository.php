		...
		try {
			...
			if ( class_exists( '\ContactInbox\Core\Logger' ) ) {
				\ContactInbox\Core\Logger::info(
					'SubmissionRepository: received data',
					array(
			...
			if ( ! empty( $data['attachment'] ) ) {
				...
				if ( is_string( $attachment ) && strlen( $attachment ) > 0 && ( $attachment[0] === '{' || $attachment[0] === '[' ) ) {
					...
					if ( json_last_error() !== JSON_ERROR_NONE ) {
						$wpdb->query( 'ROLLBACK' );
						\ContactInbox\Core\Logger::error(
							'Invalid JSON in attachment field',
							array(
					...
					if ( is_array( $decoded ) && isset( $decoded['name'] ) && ! isset( $decoded['path'] ) ) {
						$wpdb->query( 'ROLLBACK' );
						\ContactInbox\Core\Logger::error(
							'Attachment JSON missing path key',
							array(
			...
			$format = array( '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%f', '%s', '%s', '%s' );
			$result = false;
