...
final class CRMRepository {
		...
		try {
			...
			if ( $contact_id > 0 && isset( $data['status'] ) && $data['status'] === 'sent' ) {
				...
				if ( $current_status === null ) {
					...
					$update_result = $contact_repo->update( $contact_id, array( 'crm_sync_status' => 'sent' ) );
	...
	public function has_successful_sync( int $message_id ): bool {
		...
		if ( empty( $success_statuses ) ) {
			$success_statuses = array( CRMStatus::DELIVERED );
		}
		...
		$sql          = $wpdb->prepare(
			...
			array_merge( array( $message_id, 'sync' ), $success_statuses )
		);
	...
	public function get_logs(
		...
		$allowed = array( 'id', 'created_at', 'message_id', 'crm_system', 'operation', 'crm_id', 'status' );
