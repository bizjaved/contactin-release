...
final class ContactRepository {
	...
	public function update( int $id, array $data ): bool {
		...
		if ( isset( $data['salutation'] ) ) {
			$update['salutation'] = sanitize_text_field( $data['salutation'] );
		}
		if ( isset( $data['name'] ) ) {
		...
		if ( isset( $data['primary_phone'] ) ) {
			$update['primary_phone'] = sanitize_text_field( $data['primary_phone'] );
		}
		if ( isset( $data['mobile_phone'] ) ) {
			$update['mobile_phone'] = sanitize_text_field( $data['mobile_phone'] );
		}
		if ( isset( $data['home_phone'] ) ) {
			$update['home_phone'] = sanitize_text_field( $data['home_phone'] );
		}
		if ( isset( $data['other_phone'] ) ) {
			$update['other_phone'] = sanitize_text_field( $data['other_phone'] );
		}
		if ( isset( $data['source'] ) ) {
			$update['source'] = sanitize_text_field( $data['source'] );
		}
		if ( isset( $data['crm_sync_status'] ) ) {
			$update['crm_sync_status'] = sanitize_text_field( $data['crm_sync_status'] );
		}
		if ( isset( $data['last_message_at'] ) ) {
			$update['last_message_at'] = sanitize_text_field( $data['last_message_at'] );
		}
	...
	private function queue_contact_for_crm_deletion( int $contact_id, ?object $contact = null ): void {
		...
		if ( ! $delete_sync_enabled ) {
			...
			if ( class_exists( $logger_class ) ) {
				$logger_class::info(
					'CRM deletion sync disabled; GDPR log marked manual_required',
					array(
						'contact_id'      => $contact_id,
						'email'           => $contact->email,
						'messages_synced' => $messages_synced,
		...
		try {
			...
			if ( ! is_wp_error( $queue_id ) ) {
				...
				if ( $queue_status === 'completed' ) {
					...
					if ( class_exists( $logger_class ) ) {
						$logger_class::info(
							'CRM deletion queue skipped by deduplication rules',
							array(
								'contact_id'       => $contact_id,
								'email'            => $contact->email,
								'queue_id'         => $queue_id,
				...
				if ( class_exists( $logger_class ) ) {
					$logger_class::error(
						'Failed to queue contact for CRM deletion',
						array(
							'contact_id'  => $contact_id,
							'email'       => $contact->email,
			...
			if ( class_exists( $logger_class ) ) {
				$logger_class::debug(
					'Contact queued for CRM deletion',
					array(
						'contact_id'      => $contact_id,
						'email'           => $contact->email,
	...
	public function delete( int $id ): bool {
		...
		return (bool) $wpdb->delete( $this->table_contacts, array( 'id' => $id ), array( '%d' ) );
	}
	...
	public function get_paginated( int $page = 1, int $per_page = 20, string $search = '', string $orderby = 'updated_at', string $order = 'DESC' ): array {
		...
		$allowed_orderby = array( 'name', 'email', 'last_message_at', 'created_at', 'updated_at' );
		...
		return array_map( fn( $row ) => new Contact( $row ), $rows ?: array() );
	}
	...
	public function update_crm_id_if_missing( int $contact_id, string $salesforce_id ): bool {
		...
		$result = $wpdb->update(
			$this->table_contacts,
			array( 'crm_id' => sanitize_text_field( $salesforce_id ) ),
			array( 'id' => $contact_id ),
			array( '%s' ),
			array( '%d' )
		);
	...
	public function delete_with_messages( int $contact_id ): array {
		...
		if ( ! empty( $messages ) ) {
			...
			if ( class_exists( $attachment_helper_class ) ) {
				...
				if ( ! empty( $all_attachment_paths ) ) {
					...
					$attachments_deleted = count( $deletion_results['deleted'] ?? array() );
					$failed_attachments  = $deletion_results['failed'] ?? array();

					if ( ! empty( $failed_attachments ) ) {
						if ( class_exists( $logger_class ) ) {
							$logger_class::warning(
								'Some attachment files could not be deleted during contact deletion',
								array(
									'contact_id'  => $contact_id,
		...
		$messages_result  = $wpdb->delete( $table_messages, array( 'contact_id' => $contact_id ), array( '%d' ) );
		$messages_deleted = $messages_result !== false ? (int) $messages_result : 0;

		// Step 5: Delete contact
		$contact_result  = $wpdb->delete( $this->table_contacts, array( 'id' => $contact_id ), array( '%d' ) );
