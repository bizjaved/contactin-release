...
final class GDPRRepository {
	...
	private function build_log_filters( string $search, string $crm_status, string $deletion_status ): array {
		...
		return array( $where_sql, $where_params );
	}
	...
	public function get_logs( string $search, string $crm_status, string $deletion_status, int $limit, int $offset ): array {
		...
		$logs_params = array_merge( $where_params, array( $limit, $offset ) );

		return $wpdb->get_results( $wpdb->prepare( $logs_sql, $logs_params ) ) ?: array();
	}
	...
	public function get_export_rows( string $search, string $crm_status, string $deletion_status, int $limit, int $offset ): array {
		...
		return $wpdb->get_results( $wpdb->prepare( $query, $query_params ), ARRAY_A ) ?: array();
	}
	...
	public function delete_expired(): int {
		...
		foreach ( $expired_ids as $id ) {
			...
			// Delete the message
			if ( $wpdb->delete( $this->table_messages, array( 'id' => $id ), array( '%d' ) ) ) {
				++$deleted;
	...
	public function log_deletion( array $log_data ): bool {
		...
		$format = array( '%d', '%s', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%s', '%s', '%d', '%s' );
		...
		if ( $result === false ) {
			...
			Logger::log(
				Logger::ERROR,
				'GDPR deletion log insert failed: ' . $error_msg,
				array(
					'log_data'    => $log_data,
		...
		foreach ( $successful_crm_deletes as $crm_log ) {
			try {
				...
				if ( $updated !== false ) {
					++$synced_count;
					Logger::info(
						'GDPR log synced from CRM deletion log',
						array(
				...
				Logger::warning(
					'Error syncing GDPR log from CRM deletion',
					array(
						'crm_log_id' => $crm_log->id,
						'error'      => $e->getMessage(),
					)
