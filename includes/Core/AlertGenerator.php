...
final class AlertGenerator {
	...
	private static function emit_alert(
		...
		if ( ! $alerts_enabled ) {
			Logger::debug( 'Alerts disabled, skipping emission', array( 'type' => $alert_type ) );
			return;
		}

		// Check if this alert type is enabled
		$disabled_types = get_option( 'contactin_disabled_alert_types', array() );
		if ( in_array( $alert_type, (array) $disabled_types, true ) ) {
			Logger::debug( 'Alert type disabled, skipping', array( 'type' => $alert_type ) );
			return;
		}
	 ...
	 */
	public static function alert_queue_issue( string $message, string $severity = 'WARNING', array $context = array() ): void {
		try {
			Logger::log(
				strtolower( $severity ),
				'Queue Health Alert: ' . $message,
				array_merge( array( 'alert_type' => 'queue_health' ), $context )
			);

			// Store alert in options for admin dashboard
			$alerts = get_option( 'contactin_queue_alerts', array() );
