...
final class GDPR {
    ...
    public function maybe_handle_public_deletion(): void {
        ...
        if (!is_email($email)) {
            $this->render_error(__('Invalid email address.', 'contact-inbox-hub'));
            return;
        }
        ...
        if (!$message_id) {
            $this->render_error(__('This deletion link is invalid or has expired.', 'contact-inbox-hub'));
            return;
        }
    ...
    public function ajax_frontend_delete(): void {
        ...
        if (!is_email($email)) {
            wp_send_json_error(__('Invalid email address.', 'contact-inbox-hub'));
        }
        ...
        $message_id = $gdpr_repo->validate_token_get_id($token, $email);
        if (!$message_id) {
            wp_send_json_error(__('This deletion link is invalid or has expired.', 'contact-inbox-hub'));
        }
        ...
        try {
            ...
            foreach ($messages as $msg) {
                ...
                if ($path && file_exists($path)) {
                    $file_deleted = $this->delete_file_with_retry($path, 3);
                    if ($file_deleted) {
                        $deleted_files++;
                    } else {
                        $failed_files[] = $path;
                        // Log but don't fail the entire deletion
                        Logger::log(Logger::WARNING, 'Failed to delete attachment file', [
                            'path' => $path,
                            'message_id' => $msg->id
                        ]);
                    }
                ...
                if (!$message_repo->delete((int)$msg->id)) {
                    throw new \Exception('Failed to delete message ID: ' . (int)$msg->id);
                }
                $deleted_messages++;
            ...
            if ($contact) {
                if (!$contact_repo->delete((int)$contact->id)) {
                ...
                foreach ($unique_contact_ids as $contact_id) {
                    ...
                    if (!$contact_repo->delete((int)$linked_contact->id)) {
            ...
            wp_send_json_success([
                'message' => sprintf(
                    __('Successfully deleted %d messages, %d files, and %d contact records.', 'contact-inbox-hub'),
                    $deleted_messages,
                    $deleted_files,
                    $deleted_contacts
                )
            ...
            wp_send_json_error(
                sprintf(
                    __('Deletion failed: %s', 'contact-inbox-hub'),
                    $e->getMessage()
                )
            );
        }
    ...
    private function render_confirmation(array $vars = []): never {
        ...
        if ($template) {
            ...
            wp_die(__('GDPR confirmation template missing.', 'contact-inbox-hub'));
        }
        exit;
    }
    ...
    private function render_success(array $vars = []): never {
        ...
        if ($template) {
            ...
            wp_die(__('GDPR success template missing.', 'contact-inbox-hub'));
        }
        exit;
    }
    ...
    private function render_error(string $message): never {
        ...
        if ($template) {
            ...
            wp_die(__('GDPR error template missing.', 'contact-inbox-hub'));
        }
    ...
    private function locate_template(string $template_name): ?string {
        // Allow theme override: wp-content/themes/<theme>/contact-inbox-hub/gdpr/...
        $theme_template  = locate_template(['contact-inbox-hub/' . $template_name]);
        ...
        add_action('wp_enqueue_scripts', function () use ($with_script): void {
            ...
            if ($with_script) {
                ...
                wp_localize_script('contactin-gdpr-frontend', 'cinGdprFrontend', [
                    ...
                    'homeUrl' => home_url('/'),
                    'i18n' => [
                        'starting' => __( 'Starting deletion process...', Config::TEXTDOMAIN ),
                        'deleting' => __( 'Deleting messages and files...', Config::TEXTDOMAIN ),
                        'done' => __( 'Deletion complete!', Config::TEXTDOMAIN ),
                        'error' => __( 'An unknown error occurred.', Config::TEXTDOMAIN ),
                        'network_error' => __( 'Network error. Please check your connection and try again.', Config::TEXTDOMAIN ),
                        'try_again' => __( 'Try Again', Config::TEXTDOMAIN ),
    ...
    public function register_exporter(array $exporters): array {
        $exporters['contact-inbox-hub'] = [
            'exporter_friendly_name' => __('Contact Inbox Messages', 'contact-inbox-hub'),
    ...
    public function register_eraser(array $erasers): array {
        $erasers['contact-inbox-hub'] = [
            'eraser_friendly_name' => __('Contact Inbox Messages', 'contact-inbox-hub'),
    ...
    public function data_exporter(string $email, int $page = 1): array {
        ...
        foreach ($messages as $msg) {
            $data[] = [
                ...
                'group_label' => __('Contact Form Submissions', 'contact-inbox-hub'),
