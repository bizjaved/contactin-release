...
final class GDPR {
	...
	public function ajax_frontend_delete(): void {
		...
		try {
			...
			if ( $crm_queued_in_repo && $synced_count > 0 ) {
				...
				Logger::log(
					Logger::INFO,
					'CRM deletion queued from frontend GDPR link via delete_with_messages()',
					array(
						'contact_id' => $log_contact_id,
						'email'      => $email,
					)
			...
			Logger::log(
				Logger::ERROR,
				'GDPR frontend deletion failed: ' . $e->getMessage(),
				array(
					'email' => $email,
					'error' => $e->getMessage(),
				)
	...
	// Template Rendering
	// =========================================================================
	private function render_confirmation( array $vars = array() ): never {
	...
	private function render_success( array $vars = array() ): never {
	...
	private function render_error( string $message ): never {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			Logger::warning( 'GDPR error rendered', array( 'message' => $message ) );
		}
	...
	private function locate_template( string $template_name ): ?string {
		...
		$theme_template = locate_template( array( 'contactin-hub/' . $template_name ) );
	...
	public function data_eraser( string $email, int $page = 1 ): array {
		...
		foreach ( $messages as $msg ) {
			$path = AttachmentHelper::extract_file_path( $msg->attachment ?? '' );
			if ( $path && file_exists( $path ) ) {
				@unlink( $path ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged -- Intentional: file existence checked with file_exists() above.
			}
