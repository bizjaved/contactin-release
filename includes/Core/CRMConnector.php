...
final class CRMConnector {
	...
	private array $connection_config = array();
	...
	private function execute_send( array $params, ?int $message_id = null ) {
		...
		if ( $message_id && $this->crm_repo->has_successful_sync( $message_id ) ) {
			Logger::info(
				'CRM sync already delivered, skipping duplicate send',
				array(
					'message_id' => $message_id,
				)
			);
		...
		if ( ! empty( $params['attachment'] ) && $attachment_sync_enabled && ! empty( $inquiry_id ) ) {
			...
			if ( ! empty( $message_id ) ) {
				try {
					...
					Logger::warning(
						'Failed to fetch message for attachment',
						array(
							'message_id' => $message_id,
							'error'      => $e->getMessage(),
						)
			...
			$attachments = is_array( $params['attachment'] ) ? $params['attachment'] : array( $params['attachment'] );
			foreach ( $attachments as $attachment_url_or_path ) {
				...
				if ( ! is_file( $attachment_path ) ) {
					Logger::warning(
						'Attachment file not found after path conversion, skipping queue',
						array(
			...
			Logger::error(
				'Cannot queue attachments: inquiry_id is empty',
				array(
					'message_id' => $message_id,
				)
			);
	 ...
	 */
	private function log_crm_result( ?int $message_id, string $status, array $context = array() ): void {
		try {
			$response = $context['response'] ?? array();
			$log_data = array(
				'message_id'    => $message_id ?? 0,
				'crm_system'    => 'salesforce',
				'operation'     => 'sync',
	...
	private function upsert_contact( array $params, array $settings, array $headers, string $base_url, string $api_version, ?int $message_id = null ) {
		...
		$response = $this->request_salesforce(
			'PATCH',
			$endpoint,
			array(
				'headers' => $headers,
				'body'    => wp_json_encode( $contact_payload_to_send ),
			),
			$settings
		);
		...
		$status = in_array( $http_code, array( 200, 201, 204 ), true ) ? 'delivered' : 'rejected';

		// Handle HTTP 300: Multiple contacts found with same email
		if ( $http_code === 300 ) {
			Logger::info(
				'Multiple contacts found with same email, attempting name match',
				array(
					'email'    => $params['email'],
		...
		$error_message = null;
		if ( ! in_array( $http_code, array( 200, 201, 204 ), true ) && ! empty( $body_data[0]['message'] ) ) {
		...
		if ( ! in_array( $http_code, array( 200, 201, 204 ), true ) ) {
	...
	private function create_inquiry( array $params, array $settings, array $headers, string $base_url, string $api_version, string $contact_id, ?int $message_id = null ) {
		...
		$response = $this->request_salesforce(
			...
			array(
				'headers' => $headers,
				'body'    => wp_json_encode( $inquiry_payload ),
			),
			$settings
		);
	...
	public function upload_attachment_to_case_retry(
		...
		if ( ! is_wp_error( $result ) && $log_id > 0 ) {
			...
			Logger::info(
				'Attachment retry successful - logs updated',
				array(
					'log_id'             => $log_id,
	...
	private function queue_attachment_upload(
		...
		if ( ! is_file( $attachment_path ) ) {
			Logger::error(
				'Attachment file not found for queuing',
				array(
		...
		if ( $log_id <= 0 ) {
			Logger::warning(
				'Attachment log insert failed, queuing without log',
				array(
					'case_id'    => $case_id,
					'filename'   => $filename,
					'message_id' => $message_id,
		...
		$queue_id = \ContactInbox\Core\QueueManager::push(
			...
			array(
				...
				'settings'        => $this->connection_config['settings'] ?? array(),
				'headers'         => $this->connection_config['headers'] ?? array(),
		...
		if ( is_wp_error( $queue_id ) ) {
			Logger::error(
				'Failed to queue attachment upload',
				array(
