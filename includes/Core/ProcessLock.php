...
final class ProcessLock {
	...
	private static array $acquired_locks = array();
	...
	public static function acquire( string $process, int $ttl = self::DEFAULT_TTL ): bool {
		...
		if ( $success ) {
			...
			Logger::debug(
				"Lock acquired for process: {$process}",
				array(
					'ttl'      => $ttl,
					'lock_key' => $lock_key,
				)
	...
	public static function release( string $process ): bool {
		...
		if ( $result ) {
			...
			Logger::debug(
				"Lock released for process: {$process}",
				array(
					'lock_key' => $lock_key,
				)
			);
	...
	public static function force_release( string $process, int $min_age_seconds = 0 ): bool {
		...
		if ( $duration < $min_age_seconds ) {
			Logger::warning(
				'Cannot force release: lock too recent',
				array(
					'process'          => $process,
		...
		if ( $success ) {
			Logger::warning(
				'Lock force-released (possible crash recovery)',
				array(
					'process'          => $process,
					'duration_seconds' => $duration,
				)
	...
	public static function register_cleanup(): void {
		register_shutdown_function( array( self::class, 'cleanup_on_shutdown' ) );
	}
	...
	public static function get_all_locks(): array {
		$locks     = array();
		$processes = array( 'email', 'crm', 'webhook', 'analytics' );
