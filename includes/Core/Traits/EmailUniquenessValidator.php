...
trait EmailUniquenessValidator {
    ...
    public static function validate_email_uniqueness(string $email, int $exclude_contact_id = 0): array {
        ...
        if (empty($email)) {
            return ['valid' => true, 'conflict_id' => null, 'conflict_name' => null];
        }
    ...
    public static function validate_email_change(int $contact_id, string $new_email, string $current_email): array {
        ...
        if (!$validation['valid']) {
            $message = sprintf(
                __('Email "%s" is already used by contact "%s" (ID: %d). Each contact must have a unique email address.', Config::TEXTDOMAIN),
                esc_html($new_email),
                esc_html($validation['conflict_name'] ?? __('Unknown', Config::TEXTDOMAIN)),
    ...
    public static function check_email_availability(string $email, int $exclude_contact_id = 0): array {
        ...
        $message = sprintf(
            __('Email already in use by %s', Config::TEXTDOMAIN),
            esc_html($validation['conflict_name'] ?? __('another contact', Config::TEXTDOMAIN))
        );
