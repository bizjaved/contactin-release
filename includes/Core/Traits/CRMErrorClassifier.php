...
trait CRMErrorClassifier {
     ...
     */
    protected function classify_error(int $http_code, array $response = []): string {
    ...
    protected function should_retry(string $error_type, int $retry_count = 0): bool {
        $no_retry = ['AUTH', 'VALIDATION', 'FIELD_MAPPING'];
    ...
    private function is_validation_error(array $response): bool {
        ...
        if (isset($response['error']) && is_array($response['error'])) {
            ...
            $keywords = ['required', 'invalid', 'missing', 'field', 'format'];
