...
trait CRMDuplicateResolver {
	...
	protected function resolve_duplicate_contact(
		...
		Logger::info(
			'Extracted contact IDs from duplicate response',
			array(
				'email'       => $email,
		...
		if ( is_wp_error( $contacts ) ) {
			...
			Logger::warning(
				'Could not fetch duplicate contacts for name matching, using first ID',
				array(
					'email'      => $email,
		...
		if ( $best_match && $best_match_score >= 0.7 ) {
			Logger::info(
				'Found name match for duplicate contact',
				array(
					'email'        => $email,
		...
		Logger::info(
			'No name match found, using most recently modified contact',
			array(
				'email'          => $email,
