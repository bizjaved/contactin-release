...
trait AttachmentScannerTrait {
    ...
    public function find_orphaned_attachments() {
        ...
        if (is_dir($uploads_dir)) {
            ...
            $all_files = array_filter($all_files, function($f) use ($uploads_dir) {
            ...
            $referenced_normalized = array_map(function($path) {
        ...
        if (is_dir($temp_dir)) {
            ...
            $temp_files = array_filter($temp_files, function($f) use ($temp_dir) {
    ...
    public function find_stale_db_entries() {
        ...
        if (is_dir($uploads_dir)) {
            ...
            $disk_files = array_filter($files, function($f) use ($uploads_dir) {
                return is_file($uploads_dir . $f);
            });
