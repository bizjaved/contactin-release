...
final class CRMFieldMapper
{
    ...
    public static function split_name(string $fullName, string $order = 'first_last'): array
    {
        ...
        return ['first' => $first, 'last' => $last, 'valid' => true];
    }
     ...
     */
    public static function apply_mapping(array $params, array $mapping, array $crm_settings = []): array
    {
        $payload = [];
     ...
     */
    public static function map_contact_fields(array $params, array $crm_settings = [], bool $is_new_contact = true): array
    {
        $payload = [];
        $name_order = $crm_settings['name_field_order'] ?? 'first_last';
        $contact_mapping = $crm_settings['contact_mapping'] ?? [];
        ...
        if (!empty($params['phone'])) {
            ...
            if ($phone_validation === 'basic' && !self::is_valid_phone($params['phone'])) {
                ...
                Logger::warning('Phone validation failed, skipping phone field', [
    ...
    {
        $payload = [];
        $inquiry_mapping = $crm_settings['inquiry_mapping'] ?? [];
        ...
        if (!empty($subject_value)) {
            ...
            if (!empty($crm_settings['prepend_intent_to_subject']) && !empty($params['intent_category'])) {
                ...
                $subject_value = $intent_label . ': ' . $subject_value;
            }
