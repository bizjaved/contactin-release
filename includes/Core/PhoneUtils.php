...
final class PhoneUtils {
	 ...
	 */
	private static $mobile_patterns = array(
	...
	public static function bucket( array $payload, string $default_country = '' ): array {
		...
		$fields_order = array( 'mobile_phone', 'phone', 'home_phone', 'other_phone' );
		$seen         = array();
		...
		foreach ( $fields_order as $field ) {
			...
			$target = $field;
			if ( $field === 'phone' ) {
				$type = self::detect_type( $raw, array( 'default_country' => $default_country ) );
