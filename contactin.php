...
if ( ! function_exists( 'contactin_fs' ) ) {
    ...
    function contactin_fs() {
        ...
        if ( ! isset( $contactin_fs ) ) {
            ...
            $contactin_fs = fs_dynamic_init( array(
                'id'                  => '24327',
                'slug'                => 'contactin',
...
function contactin_fs_uninstall_cleanup() {
    ...
    try {
        ...
        if ( $integrity['status'] !== 'ok' ) {
            error_log( '[ContactIn Pro] Uninstall completed with warnings: ' . wp_json_encode( $integrity['missing_tables'] ) );
        }
    } catch ( \Throwable $e ) {
        // Log the error but don't let it prevent uninstallation
        error_log( '[ContactIn Pro] Fatal error during uninstall: ' . $e->getMessage() );
        error_log( '[ContactIn Pro] Error trace: ' . $e->getTraceAsString() );
    }
}
...
if ( ! file_exists( $autoloader ) ) {
	add_action(
		...
		<div class="notice notice-error is-dismissible">
			<p>
				<strong><?php esc_html_e( 'ContactIn Pro:',  'contactin'); ?></strong>
...
add_filter( 'fs_plugins_api', function( $result, $action, $args ) {
	if ( $action !== 'plugin_information' ) {
		return $result;
	}
...
add_filter( 'plugin_row_meta', function( array $links, string $file ): array {
	...
	$links = array_filter( $links, function( string $link ): bool {
	...
	$links['view-details'] = sprintf(
		...
		esc_url( $plugin_details_url ),
		esc_attr__( 'More information about ContactIn Pro',  'contactin'),
		esc_attr__( 'ContactIn Pro',  'contactin'),
...
// Load text domain at priority 0 — must run BEFORE Freemius (priority 1)
// so the domain is registered before Freemius uses it for SDK strings.
// Running at plugins_loaded priority 0 satisfies WP 6.7+ JIT requirements.
add_action(
	'plugins_loaded',
	function () {
			...
			<div class="notice notice-error">
				<p>
						...
						printf(
							...
							esc_html__( 'ContactIn Pro requires PHP 7.4 or higher. You are running PHP %s.',  'contactin'),
